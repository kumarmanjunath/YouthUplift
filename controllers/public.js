...
exports.publicPhotoUpload = asyncHandler(async (req, res, next) => {
  ...
  file.mv(
    `${__dirname}/../client/public/uploads/profile/${file.name}`,
    async (err) => {
      if (err) {
        console.error(err);
...
// @route     POST /api/v1/public/me
// @access    Private
exports.getMeUser = asyncHandler(async (req, res, next) => {
  const student = await Public.findOne({ user: req.user.id });
